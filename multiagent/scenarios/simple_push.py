...
from multiagent.scenarios.config import indexed_color, nonnegative_int, positive_int
import random
...
class Scenario(BaseScenario):
    def make_world(self, num_agents=2, num_adversaries=1, num_landmarks=2):
        ...
        if num_adversaries >= num_agents:
            raise ValueError("simple_push needs at least one non-adversary agent.")
    ...
    def reset_world(self, world):
        ...
        for i, agent in enumerate(world.agents):
            ...
            if agent.adversary:
                ...
                agent.color = np.clip(agent.color + 0.5 * goal.color, 0, 1)
