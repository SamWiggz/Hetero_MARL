...
from onpolicy.utils.balancer import Runtime_Load_Balancer
import Config
...
class GMPERunner(Runner):
    ...
    def run(self, num_cpu_processes, num_gpu_processes, tuner = False):
        self.warmup()
        ...
        mu_count = 0
        profile = 0
        timing_log_interval_updates = max(0, int(getattr(Config, "timing_log_interval_updates", 5)))
        next_timing_log_update = timing_log_interval_updates
        episodes = (
            int(self.num_env_steps) // self.episode_length // self.n_rollout_threads
        )
        balancer = Runtime_Load_Balancer(self.n_rollout_threads * self.episode_length * self.num_agents, get_device(dist.get_rank(), num_cpu_processes) == torch.device('cpu'), dist.get_rank(), dist.get_world_size(), num_cpu_processes, num_gpu_processes)
        ...
        for episode in range(self.num_episodes):
            ...
            dc_last = time.perf_counter() - dc_clock
            dc_count += 1

            # compute return and update network
            mu_clock = time.perf_counter()
            self.compute()
            train_infos, profile = self.train(balancer.get_subbatch_size())
            dist.barrier()
            mu_tot += time.perf_counter() - mu_clock
            mu_last = time.perf_counter() - mu_clock
            mu_count += 1
            balancer.update2(mu_last, profile)
            profile = 0
            #print(mu_count)
        ...
        if dist.get_rank() == 0:
            print(f"Total Train Time: {time.perf_counter() - start:.4f}s")
            ...
            print("Agents: ", self.num_agents)
            print("Batch Size: ", Config.batch_size)
        return mu_last + dc_last
