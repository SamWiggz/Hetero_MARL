...
class GR_MAPPOPolicy:
    ...
    def __init__(
        ...
        self.actor = DDP(self.actor, find_unused_parameters=True)
        self.critic = DDP(self.critic, find_unused_parameters=True)

        self.policy_timer = AllReduceTimer(dist.get_rank())
        self.critic_timer = AllReduceTimer(dist.get_rank())

        self.actor.register_comm_hook(state=None, hook=self.policy_timer.hook)
