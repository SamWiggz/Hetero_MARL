...
class GR_MAPPO():
    ...
    def __init__(self,
        ...
        if self._use_popart:
            self.value_normalizer = self.policy.critic.module.v_out
    ...
    def train(self, micro_batch:int,
        ...
        if dist.get_world_size() > 1:
            profile =  self.policy.policy_timer.get_total_time() + self.policy.critic_timer.get_total_time()
            self.policy.policy_timer.reset()
            self.policy.critic_timer.reset()
        else:
            profile = 0

        return train_info, profile
