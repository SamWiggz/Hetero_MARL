...
def is_cpu_process(rank):
    return rank < Config.cpu_processes

def gpu_mapping(rank):
    return (rank - Config.cpu_processes) % torch.cuda.device_count()

def get_device(rank):
    return torch.device("cpu" if is_cpu_process(rank)
                          else "cuda:{}".format(gpu_mapping(rank)))

def main(rank, world_size, args):
    if Config.cpu_processes == 0:
        #print(rank, "nccl")
        dist.init_process_group('nccl', rank=rank, world_size=world_size)
        torch.cuda.device(get_device(rank))
    else :
        #print(rank, "gloo")
    ...
    # cuda
    if get_device(rank) != torch.device('cpu'):
    ...
    runner.run(Config.cpu_processes, Config.gpu_process)
