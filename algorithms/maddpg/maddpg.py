...
from utils import hard_update, soft_update, gumbel_softmax, onehot_from_logits, OUNoise, enable_gradients, disable_gradients
import Config
import time
...
class MADDPG(object):
    ...
    def update(self, sample, agent_i, device, logger=None):
        ...
        ##
        # FW Target Policy
        if self.alg_types[agent_i] == 'MADDPG': # MADDPG
            ...
            if self.discrete_action:
                ...
                trgt_vf_in = torch.cat((next_obs[agent_i],
                                        curr_agent.target_policy_nn(next_obs[agent_i])),
                                    dim=1)
